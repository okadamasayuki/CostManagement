...
const items = [
  ...
  },
  {
    ...
    steps: [
      ...
      'リボンのすぐ下の<b>適用先</b>を「全ブック・全シート」に',
      '「年度更新」タブで<b>ファイル名</b>にもチェックを入れる',
      '<b>「年を +1 年ずらす」を 1 回押す</b>（30 ファイルが一度に更新）',
      '結果を何ファイルか開いて確認',
      '「元の場所へ上書き保存」（前年のファイルを残すか消すかも選べます）',
    ],
    note:
      '<b>年度更新は「年の数字」を書き換える機能です。</b>実績の数値そのものが列ごと動くわけではありません。' +
      'どの年が書いてあるかを前もって調べておく必要はなく、ツールが年に見える数字を探してまとめて進めます。' +
      '<br>数量 2,031 個のように<b>数字だけ</b>が入ったセルは、年と同じ 4 桁でも<b>既定では書き換えません</b>' +
      '（「数字だけのセルも年とみなす」のチェックは、はじめから外れています）。動画ではその点も説明しています。',
  },
];
