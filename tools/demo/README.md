...
4. 適用先を全ブック・全シートに、「年度更新」タブでファイル名も対象に
5. **数量にある「年に見える 4 桁」** の落とし穴と、既定で守られていることの説明
6. 「年を +1 年ずらす」を 1 回押す
7. 結果を 5 ファイル確認
...
`verify-both-contexts.mjs` は、動画とまったく同じ手順を

- **A**: GitHub Pages と同じ「サーバー配信」
- **B**: ダウンロードした HTML を「ダブルクリックで開いた状態 (file://)」

の両方で最後まで実行し、出来上がった 30 ファイルを突き合わせます。

```sh
node tools/demo/verify-both-contexts.mjs
```

出力される Excel が両者で完全に一致し、どちらも検査に合格することを確かめます。
同じ趣旨の確認は `npm run test:ui` の
「動画の手順が両方の起動方法で同じ結果になるか」にも入っています。
...
- **ロケール**: `LC_ALL=C.UTF-8` 相当が要ります（POSIX のままだと Chromium が
  ダウンロード名の日本語を落とします）。
- **フォルダー選択ダイアログを最後まで操作できない理由**: ダイアログ自体は
  `file://` でもサーバー配信でも同じように開きますが、フォルダーを確定した瞬間に
  この環境の Chromium が SIGTRAP で終了します（`mode: 'read'` でも同じ）。
  D-Bus を用意しても変わりませんでした。実機の Windows / macOS では起きません。
